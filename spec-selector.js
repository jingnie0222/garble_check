    ...
    */
    ...
    try {
        ...
        program.version('1.0.0')
            .description('Designed for Sogou Search Web@testing . You can use this script in the node environment. ^_^')
            .usage(' [options]')
            .option('-t, --ua-type <value>', 'Specifies the style of the browser, choise in: [ios、android、pc ]')
            .option('-m, --selector-mode <value>', 'The mode determines the positioning scheme, choise in:[css、uncss]')
        ...
        try {
            //访问指定的url地址
			//console.log("url here:" + url_decode(`${program.url}`));
        ...
        if(sMode === 'css'){
            ...
            captureStatus = await screenshotDOMElement(page, url_decode(`${program.selectorKey}`), `${program.name}`);
            ...
            captureStatus = await srceenshotFromChild(page, sKey, `${program.name}` );
        }
